...
## **************** for Marta, NZMSS -- temporary
marta <- 
  df_FarOut %>% 
  dplyr::select(date, hour, lat, lon, 
                petrel, 'storm_&_diving_petrel', count) %>%
  dplyr::rename(stormy = 'storm_&_diving_petrel') %>% 
  dplyr::filter(stormy == 'White-faced storm petrel'| 
                  stormy== 'NZ storm petrel'| 
                petrel == 'Black petrel'| 
                  petrel == 'Grey-faced petrel (Oi)'| 
                  petrel == 'Cook/Pycroft petrel')

write_csv(marta, "./marta_nzmss.csv")
write_csv(df_FarOut, "./marta_nzmss_all.csv")
